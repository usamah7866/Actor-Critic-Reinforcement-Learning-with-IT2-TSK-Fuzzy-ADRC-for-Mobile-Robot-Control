...
clear
clc
N = 220;                 % Number of Iterations
T = 0.1;                 % Sampling time (sec.)

Vr = 0.25;               % Reference Linear velocity (m/s)
Wr = 0.4;                % Reference Angular velocity (rad/s)
...
xm(1) = 0;
ym(1) = 0.6;   % Start along Y-axis (center of circle)
theta_m(1) = 0;
...
a12v(2)=1.5; a22v(2)=0.01;
a13v(2)=0.5; a23v(2)=0.5;
a14v(2)=0.5; a24v(2)=1;
a15v(2)=0.9; a25v(2)=0.9;
a16v(2)=0.2; a26v(2)=1;
...
%% ------ Updating parameters for linear --------
...
%% ------ Updating parameters for Angular --------
...
xlabel('X position (m)');
ylabel('Y position (m)');
title('Adaptive TSK ADRC');
legend('Reference Trajectory','ADRC-FPD');
axis equal;
grid on;
savefig(fullfile(outputFolder, 'Trajectory_adaptive_tsk.fig'));
...
% legend('vd','vm');
% grid on;
% savefig(fullfile(outputFolder, 'LinearVelocity_adaptive_tsk.fig'));
% 
...
% legend('wd','wm');
% grid on;
% savefig(fullfile(outputFolder, 'AngularVelocity_adaptive_tsk.fig'));
...
% grid on;
% savefig(fullfile(outputFolder, 'ThetaOrientation_adaptive_tsk.fig'));
...
% grid on;
% savefig(fullfile(outputFolder, 'MAE_PositionError_adaptive_tsk.fig'));
% 
...
% grid on;
% savefig(fullfile(outputFolder, 'X_Position_adaptive_tsk.fig'));
% 
...
% grid on;
% savefig(fullfile(outputFolder, 'Y_Position_adaptive_tsk.fig'));
